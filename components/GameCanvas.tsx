...
export const GameCanvas: React.FC<GameCanvasProps> = ({
    ...
    useEffect(() => {
        if (controlMode === 'button') {
            ...
            // If stuck in loading, go to IDLE
            if (gameStatus === GameStatus.LOADING_MODEL) {
        ...
        if (handLandmarkerRef.current) {
            // AI ready, ensure camera is running
            startCamera();
            return;
        }
    ...
    // Keyboard & Mouse Listeners for Button Mode
    useEffect(() => {
    ...
    const animate = () => {
        ...
        if (!canvasRef.current) return;
        // In gesture mode, we need video and AI. In button mode, we don't.
        if (controlMode === 'gesture' && (!videoRef.current || !handLandmarkerRef.current)) return;
        ...
        try {
            if (controlMode === 'gesture' && handLandmarkerRef.current && videoRef.current) {
                ...
                // 1. Joystick Input (Mobile priority)
                if (joystickRef.current.active) {
                    targetAngle = joystickRef.current.angle;
                    throttle = joystickRef.current.force;
                }
                ...
                const cy = ch / 2;
                lastFingerPosRef.current = {
                    x: cx + Math.cos(targetAngle) * 200,
                    y: cy + Math.sin(targetAngle) * 200
                };
            }
        } catch (e) { }
        ...
        if (controlMode === 'gesture' && videoRef.current) {
            ctx.drawImage(videoRef.current, 0, 0, cw, ch);
            ctx.fillStyle = 'rgba(0,0,0,0.6)';
        } else {
            // Button Mode Background
        ...
        if (hasHand && controlMode === 'gesture') {
    ...
    const animatePreview = () => {
        ...
        if (!canvasRef.current) return;
        // In gesture mode, wait for video. In button mode, ignore.
        if (controlMode === 'gesture' && (!videoRef.current || videoRef.current.readyState < 2)) return;
    ...
    return (
        <div className="relative w-full h-full bg-black rounded-3xl overflow-hidden border border-slate-700 shadow-2xl">
            <video
                ref={videoRef}
                className={`absolute top-4 right-4 w-32 h-24 sm:w-48 sm:h-36 object-cover rounded-lg border-2 border-white/20 shadow-lg -scale-x-100 ${controlMode === 'button' ? 'hidden' : ''}`}
                autoPlay
                playsInline
                muted
            />
            ...
            {/* Button Controls */}
            {controlMode === 'button' && gameStatus === GameStatus.PLAYING && (
                <GameControls
                    onMove={(angle, force) => {
                        joystickRef.current = { angle, force, active: true };
                    }}
                    onMoveEnd={() => {
                        joystickRef.current = { angle: 0, force: 0, active: false };
                    }}
                    onSplit={() => {
                        if (Date.now() - lastSplitTimeRef.current > 500) {
                            splitPlayer(lastFingerPosRef.current.x, lastFingerPosRef.current.y);
                            lastSplitTimeRef.current = Date.now();
                        }
                    }}
                    onEject={() => {
                        if (Date.now() - lastEjectTimeRef.current > EJECT_COOLDOWN_MS) {
                            ejectSpore(lastFingerPosRef.current.x, lastFingerPosRef.current.y);
                            lastEjectTimeRef.current = Date.now();
                        }
                    }}
                />
            )}
