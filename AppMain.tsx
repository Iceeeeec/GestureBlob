...
type GameMode = 'select' | 'single' | 'single-button' | 'online';

export default function AppMain() {
  const [mode, setMode] = useState<GameMode>('select');

  if (mode === 'single') {
    return <App onBack={() => setMode('select')} defaultControlMode="gesture" />;
  }

  if (mode === 'single-button') {
    return <App onBack={() => setMode('select')} defaultControlMode="button" />;
  }
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 w-full max-w-sm px-2">
        ...
        >
          ...
          <p className="text-xs sm:text-sm font-normal text-cyan-100 mt-1">摄像头手势控制</p>
        </button>

        <button
          onClick={() => setMode('single-button')}
          className="w-full px-6 sm:px-8 py-5 sm:py-6 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-bold text-lg sm:text-xl rounded-2xl transition-all shadow-lg shadow-emerald-500/20 active:scale-95"
        >
          <div className="flex items-center justify-center gap-3">
            <span className="text-xl sm:text-2xl">🕹️</span>
            <span>单人模式 (按键)</span>
          </div>
          <p className="text-xs sm:text-sm font-normal text-emerald-100 mt-1">摇杆/键盘控制</p>
        </button>
