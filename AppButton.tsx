...
import { GameCanvasButton } from './components/GameCanvasButton';
...
export default function AppButton({ onBack }: AppButtonProps) {
            ...
            {!isFullscreen && (
                <header className="px-3 sm:px-6 py-2 flex items-center justify-between bg-slate-900 border-b border-slate-800">
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded-full">按键模式</span>
                    </div>
                            ...
                            <div className="bg-black/50 backdrop-blur-sm rounded-lg p-2 sm:p-3 border border-white/10">
                                ...
                                <div className="space-y-0.5 sm:space-y-1">
                                    {leaderboard.slice(0, 5).map((entry, index) => (
                                        <div
                                            key={entry.id}
                                            className={`flex items-center justify-between text-[10px] sm:text-xs ${entry.isPlayer ? 'text-cyan-400 font-bold' : 'text-white/70'
                                                }`}
                                        >
                                            <div className="flex items-center gap-1 sm:gap-2">
                                                <span className="w-3 sm:w-4">{index + 1}.</span>
                                                <span className="truncate max-w-[50px] sm:max-w-[80px]">
                                                    {entry.isPlayer ? t.you : entry.name}
                                                </span>
                                            </div>
                                            <span className="tabular-nums">{entry.mass}</span>
                                        </div>
                                    ))}
                                </div>
                                {rank > 5 && (
                                    <div className="mt-1 sm:mt-2 pt-1 sm:pt-2 border-t border-white/10">
                                        <div className="flex items-center justify-between text-[10px] sm:text-xs text-cyan-400 font-bold">
                                            <div className="flex items-center gap-1 sm:gap-2">
                                                <span className="w-3 sm:w-4">{rank}.</span>
                                                <span>{t.you}</span>
                                            </div>
                                            <span className="tabular-nums">{Math.floor(score)}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
